...
const map = {
  ...
  render(snakePointsArray, foodPoint, stonePoints) {
    ...
    stonePoints.forEach((elm)=>{
      const stoneCell = this.cells[`x${elm.x}_y${elm.y}`];
...
const stones = {
  ...
  addStoneCoordinates(point) {
    this.points.push(point);
  },

  isOnPoint(point) {
    for(let stone of this.points){
...
const game = {
  ...
  init(userSettings) {
    ...
    this.counterElement = document.getElementById("pointsCounter");
  },
  ...
  reset() {
    this.stop();
    this.snake.init(this.getStartSnakeBody(this.config.settings.startTail), 'up');
    this.food.setCoordinates(this.getRandomFreeCoordinates());
    // this.sto ne.setCoordinates(this.getRandomFreeCoordinates());
    this.stones.addStoneCoordinates(this.getRandomFreeCoordinates());
    this.render();
  },

  getStartSnakeBody(length) {
    const startX = Math.floor(this.config.getColsCount() / 2);
    const startY = Math.floor(this.config.getRowsCount() / 2);
    const fullSnake = [];
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.stones.getAllCoordinates());
  },
  ...
  tickHandler() {
    ...
    if (this.stones.isOnPoint(this.snake.getNextStepHeadPoint())) {
      this.finish();
    }
  ...
  canMakeStep() {
    ...
    return !this.snake.isOnPoint(nextStepPoint) &&
        ...
        nextStepPoint.x >= 0 &&
        nextStepPoint.y >= 0 &&
        !this.stones.isOnPoint(nextStepPoint)
        ;
  },
  ...
  updatePoints(){
    this.counterElement.innerText = this.snake.getBodyLength();
  },
   resetScore(){
